...
class Value(Field):
    ...
    def parse_child_node(self, obj, node):
        ...
        for child in node.childNodes:
            ...
            child.TEXT_NODE, child.CDATA_SECTION_NODE):
...
class Dict(Field):
    ...
    def parse_child_node(self, obj, node):
        ...
        if res is dexml.PARSE_DONE:
            ...
            try:
                ...
                raise dexml.ParseError(
                    "Key field '%s' required but not found in dict value" % (
                    self.key,))
            if self.unique and key in items:
                raise dexml.ParseError(
                    "Key '%s' already exists in dict" % (key,))
            items[key] = val
            return dexml.PARSE_MORE
        else:
            return dexml.PARSE_SKIP
    ...
    def render_children(self, obj, items, nsmap):
        if self.minlength is not None and len(items) < self.minlength:
            raise dexml.RenderError(
                "Field '%s': not enough items" % (self.field_name,))
        if self.maxlength is not None and len(items) > self.maxlength:
            raise dexml.RenderError("too many items")
        if self.tagname:
            children = "".join(data for item in items.values() for data in
                               self.field.render_children(obj, item, nsmap))
            if not children:
                if self.required:
                    yield "<%s />" % (self.tagname,)
            else:
                yield children.join(
                    ('<%s>' % self.tagname, '</%s>' % self.tagname))
        else:
...
class Choice(Field):
    ...
    def __init__(self, *fields, **kwds):
        ...
        for field in fields:
            if isinstance(field, Model):
                ...
                raise ValueError(
                    "only Model fields are allowed within a Choice field")
    ...
    def render_children(self, obj, item, nsmap):
        if item is None:
            if self.required:
                raise dexml.RenderError(
                    "Field '%s': required field is missing" % (
                    self.field_name,))
        else:
